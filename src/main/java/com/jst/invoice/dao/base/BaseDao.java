...
public class BaseDao extends SqlSessionDaoSupport {
    private static ILogger log = new ILogger("INVOICE", BaseDao.class);
     ...
     *            mapper中sql语句对应的ID
     * @param params
     *            数据库字段数据，生成的sql_map中有现成的更新语句可以自动判断为空的值，跟据主ID进行少量参数修改。
     *            这样可以少传参数提高开发效率和性能
     */
    ...
    public <T> PageInfo<T> getPage(String statementId, BaseBean baseBean) {
        long beginTime = System.currentTimeMillis();
        log.info("=================开始请求执行数据库，【入参={}】", "statementId = " + statementId + " , baseBean = " + JSON.toJSONString(baseBean));
        PageHelper.startPage(baseBean.getCurrentPage(), baseBean.getPageSize());
